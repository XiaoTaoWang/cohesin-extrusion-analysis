...
FILE_REGIONS <- "/data/home/ruanlab/ly/ChIA-Drop/3.anchoring_middle/GM12878-cohesin-specific-regions_20260122_filt_25-75percentile_20260122.bed"

# RDS data paths
RDS_FILES <- list(
  "Cohesin" = "/data/home/ruanlab/ly/ChIA-Drop/2.CTCF_motif_fragment_between/GSE158897_GM12878-cohesin-pooled_comp.rds",
  "CTCF"    = "/data/home/ruanlab/ly/ChIA-Drop/2.CTCF_motif_fragment_between/GSE158897_GM12878-CTCF-pooled_comp.rds",
  "RNAPII"  = "/data/home/ruanlab/ly/ChIA-Drop/GSE158897/GSE158897_GM12878-RNAPII-pooledv2_comp.rds"
)

# Anchor buffer (expand anchors; set to 0 if S/E already precise)
# Optional small buffer to capture boundary fragments
ANCHOR_BUFFER <- 0

# Output directory in project root
OUTPUT_DIR <- "/data/home/ruanlab/ly/K562_chromatin_folding/loading_site_analysis_v2"
if (!dir.exists(OUTPUT_DIR)) {
  dir.create(OUTPUT_DIR, recursive = TRUE)
}

OUTPUT_PDF    <- file.path(OUTPUT_DIR, "loading_site_distribution_v2_boxplot.pdf")
INTERMEDIATE_RDS <- file.path(OUTPUT_DIR, "ChIA_Drop_Predefined_Regions_Stats.rds")
...
# Merge to create analysis pairs (Loading linked to S and E)
# Each loading site is an independent analysis row linked to its loop S and E
...
# Compute loop search range (S_start to E_end)
analysis_pairs[, `:=`(
...
if (file.exists(INTERMEDIATE_RDS)) {
  ...
  # Find all fragments within the loop range
  # Note: using pair_id to build GRanges; overlaps may occur and are handled by findOverlaps
  ...
  # 3.4 State flags relative to this pair
  dt_merged[, `:=`(
  ...
  # Fill missing pairs with zeros (count = 0)
...
if (length(final_stats_list) > 0) {
  ...
  median_stats <- plot_df_clean[, .(
    median_val = median(log_val, na.rm = TRUE),
    median_actual_value = 10^median(log_val, na.rm = TRUE), 
    median_label = sprintf("%.2f", 10^median(log_val, na.rm = TRUE)) 
  ), by = c("Type", "Sample")]

  p <- ggplot(plot_df_clean, aes(x=Type, y=log_val)) +
    geom_boxplot(color="black", fill="white", outlier.shape=1, outlier.size=1, width=0.5, lwd=0.6, fatten=0) +
    stat_summary(fun.data = function(x) c(y=median(x), ymin=median(x), ymax=median(x)),
                 geom="errorbar", aes(color=Sample), width=0.5, lwd=0.6) +
    facet_wrap(~Sample, nrow=1) +  # shared y-scale across samples
    stat_compare_means(comparisons = comparisons, label = "p.signif",
                       method="wilcox.test", step.increase = 0.14) +
    geom_text(data=n_stats, aes(x=2, y=Inf, label=paste0("n=", N)),
              inherit.aes=FALSE, vjust=1.6, hjust=0.5, size=4.5) +
    ...
    theme(
      ...
      plot.margin = margin(t = 24, r = 12, b = 22, l = 14, unit = "pt")
    )

  median_text_list <- list()
  for (sample in unique(median_stats$Sample)) {
    sample_data <- median_stats[Sample == sample]
    label_parts <- sprintf("%s: %s", as.character(sample_data$Type), sample_data$median_label)
    label_str <- paste(label_parts, collapse = "  |  ")
    median_text_list[[sample]] <- label_str
  }

  all_median_labels <- paste(unlist(median_text_list), collapse = "\n")

  median_grob <- textGrob(
    label = all_median_labels,
    gp = gpar(fontsize = 11, fontface = "bold", col = "darkred"),
    hjust = 0.5,
    vjust = 0.5
  )
  ...
  ggsave(OUTPUT_PDF, combined_plot, width=12, height=4.5, bg="white")
