...
def run_hct116_analysis():
    bed_file = f"{PROJECT_ROOT}/mESC/loading_site_define_LY_0311/mesc-cohesin-specific-regions_filt_25-75percentile.bed"
    base_out_dir = f"{PROJECT_ROOT}/mESC/ADA"
    ...
    ctcf_0h = f"{PROJECT_ROOT}/mESC/GSE178982_CTCF-UT_pool.mcool::/resolutions/2000"
    ctcf_6h = f"{PROJECT_ROOT}/mESC/GSE178982_CTCF-AID_pool.mcool::/resolutions/2000"
    rad21_0h = f"{PROJECT_ROOT}/mESC/GSE178982_RAD21-UT_pool.mcool::/resolutions/2000"
    rad21_6h = f"{PROJECT_ROOT}/mESC/GSE178982_RAD21-AID_pool.mcool::/resolutions/2000"
    
    # 1. Single-condition comparisons (6h vs 0h)
    comparisons = [
    ...
    for comp_name, path_A, name_A, path_B, name_B in comparisons:
        ...
        # --- Log2 diff unchanged (global mean normalization already applied) ---
        ...
        # --- Linear diff with diagonal scaling normalization ---
        diag_mean_A = np.nanmean(np.diag(mat_A))
        diag_mean_B = np.nanmean(np.diag(mat_B))
        scale_factor = diag_mean_B / diag_mean_A  # scale A (6h) relative to B (0h)
    ...
    # 2. Double-difference comparisons (RAD21 6h/0h vs CTCF 6h/0h)
