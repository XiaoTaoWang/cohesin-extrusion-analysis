...
# Date:        2026-01-31
# Modified:    2026-02-15 - Fixed boundary checking consistency
# ==============================================================================
...
get_interaction_stats <- function(df, left_range, right_range) {
  ...
  stats <- df_sub %>%
    ...
    # simple comment
    filter(
      (is_anchored_left & c_end > l_end & c_end <= r_end) |   # simple comment
      (is_anchored_right & c_start >= l_start & c_start < r_start)  # simple comment
    ) %>%
    summarise(
      complete_count = sum(has_plus_total & has_minus_total),  # simple comment
      ongoing_count = sum((has_plus_total & !has_minus_total) | (!has_plus_total & has_minus_total))  # simple comment
    )
...
process_sample_plot <- function(sample_name, config) {
  ...
  # Add title to the individual fragment plot
...
output_file <- "ChIA_Drop_Combined_CTCF_Cohesin_RNAPII.pdf"
